...
elseif Ch==ASC("R") .or. Ch==ASC("r")
  ...
  IF nGetP0>0
    ...
    DO WHILE .t.
      ...
      ELSE
        ++nGetTekStrana
      ENDIF
      IF nGetTekStrana<1; nGetTekStrana:=1; ENDIF
      IF nGetTekStrana>nGetStrana; EXIT; ENDIF
    ENDDO

    IF LASTKEY()==K_ESC
      SELECT (nPrArr); RETURN DE_CONT
    ENDIF
  ENDIF
  ...
  IF RJES->idpromj=="G1"      
    ...
    DO WHILE !EOF() .and. id==K_0->id .and. idpromj=="G1"
      IF nAtr1==ID06     // ID06 je sad za sad godina prava, kao i nAtr1
        IF nAtr2>0
          nImaDana:=nAtr2
          IF FIELDPOS("NATR3")>0
            nGOKrit1:=nAtr3
  ...
  IF nVecPostoji>0
    ...
    FOR i:=1 TO LEN(aNezelim)
      cPom:=aNezelim[i]       // ispraznimo nezeljene
      &cPom:=BLANK(&cPom)     // varijable
...
// -----------------------------------------
function rpt_rjes()
 LOCAL aPom:={},i,nLin,nPocetak,nPreskociRedova,cLin,cPom
  START PRINT CRET
  gp10cpi()
  IF EMPTY(RJES->fajl)
      ...
      DO WHILE .t.
        ...
        IF nPom>0
          ...
          IF !EMPTY(aPom[1])
            PrnKod_ON(aPom[1])
          ENDIF
          IF aPom[1]=="K"          // ako evaluacija vrsi i stampu npr.
            cPom:=&(aPom[2])       // ako je aPom[2]="gPU_ON()"
          ELSE
            cPom:=&(aPom[2])
            ?? cPom
          ENDIF
...
FUNCTION UzmiVar(cVar)
 ...
 SELECT DEFRJES
 SEEK RJES->id+cVar
 IF FOUND()                           // tip slova je string sacinjen od
   cVrati := { tipslova , iizraz }    // slova U,I i B
 ENDIF                                // (U-underline,I-italic,B-bold)
RETURN cVrati
...
function EdPromj(ch)
LOCAL lPom:=.f.
if Ch==K_ENTER .and. !(KLevel $ "01")
   Msg("Vi ne mozete mijenjati promjene !",15)
   return DE_CONT
endif
...
do case
  case Ch==K_ENTER .or. Ch=K_CTRL_N
     ...
     @ m_x+3,m_y+2    SAY  "Tip promjene  "  GET qIdPromj  VALID P_Promj(@qIdPromj,3,40)  PICTURE "@!"
     @ m_x+4,m_y+2    SAY  "Karakteristika"  GET qIdK      PICTURE "@!"
     read

     if qIdPromj=="G1"     // godisnji odmor
       @ m_x+6,m_y+2 SAY "Koristi pravo na godisnji odmor za godinu   :"  GET qnAtr1  PICTURE "9999"
       @ m_x+7,m_y+2 SAY "Broj dana godisnjeg odmora na koji ima pravo:"  GET qnAtr2  PICTURE "9999"
       IF !("U" $ TYPE("qnAtr3"))
     ...
     if qDatumOd>=_DatURMJ // ako se ubacuje stara promjena ovaj uslov
      qIdRJ:=_IdRJ         // nije zadovoljen
      qIdRMJ:=_IdRmj
     endif

     if P_PROMJ(qIdPromj,-6)=="1"  // srj="1" ako se mijenja promjenom radno mjesto
      @ m_x+6,m_y+2   SAY  "RJ"  GET qIdRj PICTURE "@!"
     ...
     if (cTipPromj:=P_PROMJ(qIdPromj,-4)) == "X" .and. P_Promj(qIdPromj,-7) $ "+-*A=" // -7 = URst
     ...
     if P_PROMJ(qIdPromj,-8)=="1" // u Ratni raspored
     ...
     if P_PROMJ(qIdPromj,-9)=="1" // u strucnu spremu
     ...
     if lastkey()<>K_ESC
        Gather("q")
        if (Ch==K_CTRL_N .and. Pitanje("p09","Zelite li azurirati ovu promjenu ?","D")=="D") .or.;  // "filovanje" kartona sa stanjem promjene
           (Ch==K_ENTER .and. Pitanje("p10","Zelite li ponovo azurirati ovu promjenu ?","N")=="D")

           if P_PROMJ(qIDPromj,-4)<>"X" // Tip X - samo postavlja odre|ene parametre
           ...
           if P_PROMJ(qIdPromj,-4)=="I"  // intervalna promjena
               _DatVRMJ:=qDatumOd
               replace DatumDo with CTOD("") // "otvori promjenu !"
           endif

           if P_PROMJ(qIDPromj,-8)=="1"    // uRRasp = 1
              _IdRRasp:=qcAtr1
           endif
  ...
  case Ch==K_ALT_K
     if P_PROMJ(IdPromj,-4)="I" .and. empty(DatumDo) // intervalna promjena
        dPom:=DATE()
        Box("bzatv",3,40,.f.)
        set cursor on
        ...
        if lastkey()<>K_ESC
          ...
          if P_PROMJ(IdPromj,-5)="M" .and. P_RRASP(left(cAtr1,4),-4)="V" // sluzenje vojnog roka
             _SlVr:="D"
